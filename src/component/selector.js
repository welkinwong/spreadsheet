...
class SelectorElement {
  constructor() {
    this.cornerEl = h('div', 'spreadsheet-selector-corner');
    this.areaEl = h('div', 'spreadsheet-selector-area')
      .child(this.cornerEl)
      .hide();
    this.clipboardEl = h('div', 'spreadsheet-selector-clipboard').hide();
    this.autofillEl = h('div', 'spreadsheet-selector-autofill').hide();
    this.el = h('div', 'spreadsheet-selector')
...
export default class Selector {
  constructor(data) {
    ...
    this.range = null;
    this.arange = null;
    this.el = h('div', 'spreadsheet-selectors')
  ...
  set(rowIndex, colIndex, indexesUpdated = true) {
    const { data } = this;
    const cellRange = data.calSelectedRangeByStart(rowIndex, colIndex);
    const { sri, sci } = cellRange;
    if (indexesUpdated) {
      let [cri, cci] = [rowIndex, colIndex];
      if (rowIndex < 0) cri = 0;
      if (colIndex < 0) cci = 0;
  ...
  setEnd(rowIndex, colIndex, moving = true) {
    const { data, lastri, lastci } = this;
    if (moving) {
      if (rowIndex === lastri && colIndex === lastci) return;
      this.lastri = rowIndex;
      this.lastci = colIndex;
    }
    this.range = data.calSelectedRangeByEnd(rowIndex, colIndex);
  ...
  showAutofill(rowIndex, colIndex) {
    if (rowIndex === -1 && colIndex === -1) return;
    // console.log('rowIndex:', rowIndex, ', colIndex:', colIndex);
    // const [sri, sci] = this.sIndexes;
    // const [eri, eci] = this.eIndexes;
    const { sri, sci, eri, eci } = this.range;
    const [nri, nci] = [rowIndex, colIndex];
    // const rn = eri - sri;
    // const cn = eci - sci;
    const srn = sri - rowIndex;
    const scn = sci - colIndex;
    const ern = eri - rowIndex;
    const ecn = eci - colIndex;
    if (scn > 0) {
      // left
      // console.log('left');
